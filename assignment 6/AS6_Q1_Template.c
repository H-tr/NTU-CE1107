...
int HashInsert(HashTable* Q1Ptr, int key)
{
    ...
    //check whether the key is already in the hash table
    if (HashSearch(*Q1Ptr, key) != NULL)
        return 0;
    else {
        Q1Ptr->Table[index].size++;
        cur = malloc(sizeof(ListNode));
        cur->next = Q1Ptr->Table[index].head;

        if (Q1Ptr->Table[index].head)
            Q1Ptr->Table[index].head->pre = cur;
        Q1Ptr->Table[index].head = cur;
        cur->key = key;
        cur->pre = NULL;
...
int HashDelete(HashTable* Q1Ptr, int key)
{
    ...
    //check whether there is a key in the hash table
    if (temp != NULL) {
        //check whether the key is the first node
        if (temp->pre == NULL) {
            Q1Ptr->Table[Hash(key, Q1Ptr->hSize)].head = temp->next;
            free(temp);
        }
        else {
            cur = temp->pre;
